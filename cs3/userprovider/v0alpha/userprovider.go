...
service UserProviderService {
  ...
  rpc GetUserGroups(GetUserGroupsRequest) return (GetUserGroupsResponse);
  // Tells if the user is in a certain group.
  rpc IsInGroup(IsInGroupRequest) return (IsInGroupResponse);
...
message FindUsersResponse {
  ...
  // REQUIRED.
  // The groups for the user.
  repeated cs3.types.User users = 3;
}
